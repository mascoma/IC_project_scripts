import re 
def main():
    output = open("ICW_unmerged_R1_tag.txt", 'w')
    readslist1 = []
    readslist2 = []
    readslist = []
    try:
        f1 = open("ICW_R1_paired_tag.txt", 'r')
        f2 = open("ICW_merged_tag.txt", 'r')
        print("successful open!")
        for line1 in f1:
            readslist1.append(line1.rstrip('\n'))   
        print("total reads of ICW_paired_R1_tag: ",len(readslist1))
        for line2 in f2:
            readslist2.append(line2.rstrip('\n'))   
        print("total reads of ICW_merged_tag: ",len(readslist2))
