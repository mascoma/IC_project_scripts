...
outdir <- "/isi/olga/xin/Halophile_project/output/20160426/"
outputfile1 <- "IC_DS2_1_genus.jpeg"
outputfile2 <- "IC_DS2_2_genus.jpeg"
DS2_1.map.dat <- slice_data(dir, inputfile1, 13, "not_eaten")
DS2_1.unmap.dat <- slice_data(dir, inputfile2, 30, "eaten")
DS2_2.map.dat <- slice_data(dir, inputfile3, 13, "not_eaten")
DS2_2.unmap.dat <- slice_data(dir, inputfile4, 30, "eaten")
IC.dat <- slice_data(dir, inputfile5, 12, "IC37")
genus.DS2_1.dat <- rbind(IC.dat, DS2_1.map.dat, DS2_1.unmap.dat) 
genus.DS2_1.dat[, 3] <- as.numeric(as.character(genus.dat[, 3]))

genus.DS2_2.dat <- rbind(IC.dat, DS2_2.map.dat, DS2_2.unmap.dat) 
genus.DS2_2.dat[, 3] <- as.numeric(as.character(genus.dat[, 3]))
...
g <- ggplot(genus.DS2_1.dat, aes(x = group, y = percent, fill = taxa)) +
...
ggsave(path = outdir, plot = g, file = outputfile1, device = "jpeg", 
       width = 20, height = 10)


g <- ggplot(genus.DS2_2.dat, aes(x = group, y = percent, fill = taxa)) +
  geom_bar(stat = "identity", color = "black", width = 0.2) + coord_flip() +
  theme(legend.text = element_text(size = 20, face="italic"), 
        legend.title=element_blank(),
        axis.text.y = element_text(size = 20), 
...
ggsave(path = outdir, plot = g, file = outputfile2, device = "jpeg", 
       width = 20, height = 10)
