...
import pandas as pd
def main():
    inputdir1 = '/Users/Xin/Desktop/IC_project/output/Jan202016/DS2_2_read_species.csv'
    ...
    cursor.execute(sql)
    sql = """CREATE TABLE IF NOT EXISTS DS2_2_species_assignment
    (
    ...
    for i in range(0, len(df1['read'])):
        r = df1['read'][i]
        t = df1['taxa'][i]
        sql = 'INSERT INTO DS2_2_species_assignment (reads_name, taxa) VALUES('+"'"+r+"'"+','+"'"+t+"'"+');'
        #print(sql)
        cursor.execute(sql)
        conn.commit()
    cursor.close()
