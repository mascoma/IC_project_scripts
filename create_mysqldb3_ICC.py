...
def main():
    ...
    cursor.execute(sql)
    sql = """CREATE TABLE IF NOT EXISTS ICC_trimmed_merged_fq
    (
    ...
    cursor.execute(sql)
    inputdir1 = '/Users/Xin/Desktop/IC_project/input/merged_reads_CLC/ICC_CLC.fastq'
    f = open(inputdir1,'r')
    tmp1 = []
    for line in f:
        if line.startswith("@M00704"):
            ...
            if tmp2:
                ...
                #print(reads_name)
                #print(seq)
                sql = 'INSERT INTO ICC_trimmed_merged_fq (reads_name, sequence) VALUES('+'"'+reads_name+'"'+','+'"'+seq+'"'+')'
                #print(sql)
                cursor.execute(sql)
                conn.commit()
            tmp1 = []
