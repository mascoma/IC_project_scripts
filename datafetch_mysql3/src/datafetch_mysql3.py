...
#     cursor.execute(sql)
    ...
    sql = 'SELECT reads_name, sequence FROM reads_trimmed_merged.ICC_trimmed_merged_fa WHERE SUBSTRING(reads_name,2) IN (SELECT reads_name FROM reads_assignment.ICC_genus_assignment) AND id < 100;'
    cursor.execute(sql)
